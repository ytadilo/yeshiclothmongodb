...
(async function () {
    ...
    function renderUserTable() {
        ...
        userTableBodyEl.innerHTML = paged.map((row) => {
            const firstVisit = row.firstVisit ? new Date(row.firstVisit).toLocaleString() : 'ΓÇö';
            const lastActive = row.lastActive ? new Date(row.lastActive).toLocaleString() : 'ΓÇö';
            ...
            return `
                <tr>
                    ...
                    <td style="padding:8px; border-bottom:1px solid rgba(0,0,0,0.06);">${escapeHtml(row.deviceType || 'ΓÇö')}</td>
