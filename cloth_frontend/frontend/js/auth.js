...
function redirectAfterAuth(user) {
    ...
    if (role === 'admin') {
        const token = String(localStorage.getItem('token') || '').trim();
        window.location.replace(token ? '/admin/orders?token=' + encodeURIComponent(token) : '/admin/orders');
        return;
    }
...
function initLoginForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (googleContainer) googleContainer.querySelector('button') && (googleContainer.querySelector('button').disabled = true);
        try {
            ...
            if (googleContainer) googleContainer.querySelector('button') && (googleContainer.querySelector('button').disabled = false);
